...
|------|:---:|:---:|:---:|:---:|:---:|
| **涨停策略 E4（去跌停+宽市值+8%止损）** | **6.93** | **382.2%** | **-21.0%** | — | 日频 |
| **大小票 v4.0（涨停+ML）** | 1.39 | 54.7% | 46.8% | +311% | 混合 |
| 涨停策略 E3 | 3.75 | 205.8% | -35.0% | — | 日频 |
...
- **E4 优化**：去掉跌停条件 + 市值放宽到 30-500 亿 + 股价上限 63 元
- 2423 笔交易分析发现涨停 7+ 次胜率仅 23%，但赢时收益巨大（非对称结构）
- 五层规则权重实验：跌停条件反而不利（去掉后 Sharpe 6.89），市值区间太窄
