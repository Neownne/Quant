#!/usr/bin/env python
"""标签烙印型再启动策略 —— 涨停池信号 + 标签烙印重排 + T+2买入窗口。

用法:
    python scripts/bt_label.py --signals data/signals/bt_signals_label.csv --top-n 5
    python scripts/bt_label.py --start 2020-01-01 --top-n 5
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="标签烙印型再启动策略")
    ...
    p.add_argument("--signals", default="data/signals/bt_signals_label.csv", help="信号CSV路径")
    p.add_argument("--min-score", type=int, default=0, help="最低标签烙印评分")
    p.add_argument("--trailing-stop", type=float, default=0.12)
    p.add_argument("--min-hold-days", type=int, default=7)
    p.add_argument("--max-wait-days", type=int, default=2, help="买入最多等几个交易日")
    return p.parse_args()
...
def run_backtest(args):
    ...
    for sig_date, sigs in sig_by_date_raw.items():
        ...
        for _, s in sigs.iterrows():
            code = s["code"]
            bought = False
            for offset in range(1, args.max_wait_days + 1):
                nxt = idx + offset
                ...
                break
            if not bought:
                logger.debug(f"  {code} {sig_date.date()} 后{args.max_wait_days}日无买入窗口")
