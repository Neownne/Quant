...
def vol_of_vol(df: pd.DataFrame) -> pd.Series:
    ...
    vol5 = ret.rolling(w(5)).std()
    return -vol5.rolling(w(20)).std()
...
def down_vol_ratio(df: pd.DataFrame) -> pd.Series:
    ...
    down_std = ret_neg.rolling(w(20)).std()
    total_std = ret.rolling(w(20)).std().replace(0, np.nan)
...
def tail_risk(df: pd.DataFrame) -> pd.Series:
    ...
    ret = df["close"].pct_change()
    return -ret.rolling(w(60)).quantile(0.05)
...
def beta_20(df: pd.DataFrame) -> pd.Series:
    ...
    cov = ret.rolling(w(20)).cov(mkt)
    var = mkt.rolling(w(20)).var().replace(0, np.nan)
    return cov / var
...
def ret_asymmetry(df: pd.DataFrame) -> pd.Series:
    ...
    ret = df["close"].pct_change()
    pos = ret.clip(lower=0).rolling(w(20)).mean()
    neg = ret.clip(upper=0).abs().rolling(w(20)).mean()
    std = ret.rolling(w(20)).std().replace(0, np.nan)
    return (pos - neg) / std
