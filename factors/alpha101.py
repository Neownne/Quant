...
def rev_5(df: pd.DataFrame) -> pd.Series:
    ...
    return -_ts_roc(df["close"], 5)


def rev_10(df: pd.DataFrame) -> pd.Series:
    """10日反转"""
    return -_ts_roc(df["close"], 10)


def rev_20(df: pd.DataFrame) -> pd.Series:
    """20日反转"""
    return -_ts_roc(df["close"], 20)
...
def vol_ratio_5_20(df: pd.DataFrame) -> pd.Series:
    """5日均量 / 20日均量"""
    v = df["volume"]
    return _ts_sum(v, 5) / _ts_sum(v, 20)
...
def up_day_ratio(df: pd.DataFrame) -> pd.Series:
    """20日中上涨天数比例"""
    return (df["close"].diff() > 0).rolling(20, min_periods=1).mean()
...
def vol_swing(df: pd.DataFrame) -> pd.Series:
    ...
    v = df["volume"]
    vol_ratio = v / v.rolling(20).mean() - 1
